...
public class adminstrator extends user{
	...
	public void edit(user u){
		...
		if(search_books.size() != 0){
			...
			int result = JOptionPane.showConfirmDialog(null, myPanel,"搜尋",  JOptionPane.YES_NO_OPTION, JOptionPane.QUESTION_MESSAGE);
			if(result == 0) {
				...
				do{
					select = JOptionPane.showOptionDialog(null,"請問您要","書籍管理", JOptionPane.YES_NO_OPTION, JOptionPane.QUESTION_MESSAGE, null,login,null);
					switch (select){
		         		case 0:
		            		revise(adbook);
		    				break;
	...
	public void revise(book b){
			...
			myPanel.add(Box.createHorizontalStrut(15));
      		...
      		int result = JOptionPane.showConfirmDialog(null, myPanel,"修改", JOptionPane.OK_CANCEL_OPTION);
      		if (result == JOptionPane.OK_OPTION) {
      			int check = 0;
      			for(book bb : books) {
