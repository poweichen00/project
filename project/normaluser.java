...
public abstract class normaluser extends user{
	...
	public void payment() {
		int i = Integer.parseInt(JOptionPane.showInputDialog(null, "請輸入欲繳納罰金","罰金繳納系統系統",1));
		if(money > i) {
			money -= i;
		}
		else {
			JOptionPane.showMessageDialog(null, "輸出超過繳納金額，請重新輸入","還書系統",1);
			payment();
		}
	}
		...
		if (userbooks.size() >= lendup) {
			...
			if (adbook.getsituation() == "unavailable") {
				...
				if (you_borrow == 1) {
					...
					int select = JOptionPane.showOptionDialog(null, "您要預約嗎？", null + "學生",
